> **Audit Status: 2026-03-22** -- Overall: Core vision achieved. Transport boundary, message envelope, and Ash notifier pattern are all implemented. Key structural deviations: Topics module replaced by flat atom naming scheme; Bus module name taken by agent message routing; emit/2 replaces new_message+publish; Message struct field set evolved. Rules 1, 2, and 4 have known violations documented below.
...
> **Status: PARTIALLY DONE** -- field set evolved in implementation. Present in actual struct: kind, signal, category, name, source, data, ref, meta, timestamp. NOT present: topic (removed -- topic derived from category/signal at broadcast time), resource, action, tenant_id, actor_id, correlation_id, causation_id. Added: name (string label), source (emitter atom), ref (optional opaque ref). timestamp: monotonic integer via System.monotonic_time(), not DateTime.t(). @enforce_keys differ from spec below. Principles (one envelope, kind field) are intact.
...
> **Status: SUPERSEDED** -- Ichor.Signals.Topics module was not created. Topic naming replaced by a flat atom-based scheme: category (atom) + signal (atom) + optional scope id, resolved at emit time inside Ichor.Events.Runtime. The string-building helpers (entity/3, collection/2, tenant/2, process/1, ui/1) do not exist. Rule 1 is the direct casualty -- topic strings are sometimes built inline (e.g., "signals:feed" raw string in at least one callsite). The principle (no raw topic strings scattered across the app) remains valid as a goal.
...
> **Status: SUPERSEDED** -- Ichor.Signals.Bus was not created as the PubSub wrapper. The PubSub role lives in Ichor.Events.Runtime (subscribe/2, broadcast internal). The name "Bus" was claimed by Ichor.Messages.Bus (agent message routing), which is a different concern. Rule 2 is the casualty -- PubSub calls are not exclusively gated through one Bus module. The instrumentation/tracing observation in the Notes below remains valid as a future hook.
...
> **Status: PARTIALLY DONE** -- Public API exists but is leaner than specified. emit/2 (category, signal, opts) replaces new_message/7 + publish/1 -- callers do not build Message structs directly. subscribe/1 and unsubscribe/1 exist on Ichor.Events.Runtime (not Ichor.Signals). The new_message/7 function was never created. put_timestamp uses System.monotonic_time() not DateTime.utc_now(). Principle (one public entrypoint) is intact.
...
> **Status: PARTIALLY DONE** -- The principle is valid and followed. Actual implementation uses per-domain emit/2 calls directly rather than named helper modules like Ichor.Billing.Signals. The examples below are stale (use the superseded new_message/7 API and Topics module) but the design principle -- thin domain helpers, no dedicated event modules -- is intact and should guide any future expansion.
...
> **Status: DONE** -- Implemented as Ichor.Signals.FromAsh, registered as an Ash.Notifier on relevant resources. Translates Ash notification structs into Ichor signal envelopes before broadcasting. The adapter pattern (dumb, explicit, not a logic hub) is faithfully followed.
...
> **Status: MOSTLY DONE** -- Rules 3, 5, 6, 7, 8, 9, 10 are followed. Known violations: Rule 1 violation -- "signals:feed" raw topic string exists in at least one callsite outside any Topics module (Topics module was never built). Rule 2 violation -- Ichor.Messages.Bus (agent routing) calls PubSub directly; Runtime also contains PubSub calls; no single Bus wrapper gates all PubSub access. Rule 4 superseded -- signal identity in practice is category + signal (flat atoms), not kind + domain + resource + action tuple. The principle (stable identity without module-per-event) is intact; the specific fields changed.

**Rule 1** -- Only Ichor.Signals.Topics builds topic strings.

**Rule 2** -- Only Ichor.Signals.Bus talks directly to Phoenix.PubSub.

**Rule 3** -- All published signals must use Ichor.Signals.Message.

**Rule 4** -- Signal identity is always: kind, domain, resource, action.

**Rule 5** -- Payload goes in data as a normalized map.

**Rule 6** -- Ash notifications are translated in Ichor.Signals.FromAsh. They are not the app-wide contract.

**Rule 7** -- Use domain helper modules like Ichor.Billing.Signals only when they remove duplication.

**Rule 8** -- Do not create dedicated event modules unless the event is truly special.

**Rule 9** -- Do not let LiveViews, workers, or services invent topic names locally.

**Rule 10** -- Keep Ichor.Signals as a protocol layer. Do not let it absorb business logic from every domain.
...
> **Status: STILL RELEVANT** -- ideas from the original vision not yet implemented, worth revisiting.

**Entity-level topic subscriptions**
`scoped/3` exists in the topic scheme but is barely used. The consumer example above (subscribing
to a specific invoice by id) shows the intended pattern. As the app grows, more entities will
benefit from fine-grained subscriptions rather than category-level fan-out.

**tenant_id and actor_id on the envelope**
The original Message struct included `tenant_id` and `actor_id`. These were dropped in the
simplified implementation. When multi-tenancy becomes a first-class concern, adding them back
to the envelope (or to a `meta` sub-key) will be the right place -- not scattered across payloads.

**correlation_id / causation_id for distributed tracing**
`correlation_id` and `causation_id` were in the original spec and would enable request tracing
across signal chains (e.g., which Oban job triggered which signal, which signal triggered which
LiveView update). Not populated today. The fields are known and the pattern is ready; it requires
a propagation convention through the call stack.

**Telemetry hook in Runtime**
`Ichor.Events.Runtime` handles all broadcast. It is the correct place to emit a
`:telemetry.execute/3` call on every signal (for metrics, tracing, alerting). The hook is not
yet wired. Adding it here keeps instrumentation centralized, matching the original Bus intent.
