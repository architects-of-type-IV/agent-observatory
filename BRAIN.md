...
- **embed_templates file split pattern**: Move preprocessing into `<% %>` blocks at top of .heex templates. Keep multi-head pattern-matched dispatch functions (e.g., `segment/1`, `role_badge/1`) as manual `defp` in the .ex file. Keep helper functions in .ex. Templates generated by embed_templates are called directly from dispatch functions: `defp segment(%{segment: %{type: :parent}} = assigns), do: parent_segment(assigns)`.
- **Recursive component rendering**: A component can call itself with different args. Multi-tool chain renders `<.tool_chain :for={pair <- @pairs} pairs={[pair]} .../>` to render each individual tool via the same component's single-tool path.
- **Timeline interleaving**: Instead of rendering all tool pairs then all standalone events, use `build_segment_timeline/2` to interleave both chronologically, then group consecutive tool items into `{:tool_chain, pairs}` tuples. Preserves temporal context.
- **Format-on-save race condition**: Edit tool fails with "file modified since read" when format-on-save hooks run between Read and Edit. Use `sed -i ''` via Bash as workaround for simple replacements.
