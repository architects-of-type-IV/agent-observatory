  ...
  def forensic_view(assigns) do
    ...
    <div id="forensic-view" class="p-6 space-y-6">
      ...
      <div id="message-archive-panel" class="bg-base/50 border border-border rounded-lg p-4">
        <h3 class="text-sm font-semibold text-default uppercase tracking-wider mb-3">Message Archive</h3>
        ...
        <% else %>
          <div :if={@archive_results != []} class="space-y-1 max-h-48 overflow-y-auto">
            <div :for={result <- Enum.take(@archive_results, 50)} class="text-xs font-mono text-default truncate">
      ...
      <div id="cost-attribution-panel" class="bg-base/50 border border-border rounded-lg p-4">
        <div class="flex items-center justify-between mb-3">
          <h3 class="text-sm font-semibold text-default uppercase tracking-wider">Cost Attribution</h3>
      ...
      <div id="security-panel" class="bg-base/50 border border-border rounded-lg p-4">
        <h3 class="text-sm font-semibold text-default uppercase tracking-wider mb-3">Security Webhook Log</h3>
        <%= if @webhook_logs == [] do %>
      ...
      <div id="policy-engine-panel" class="bg-base/50 border border-border rounded-lg p-4">
        <h3 class="text-sm font-semibold text-default uppercase tracking-wider mb-3">Policy Engine</h3>
        <%= if @policy_rules == [] do %>
          <p class="text-sm text-low mb-3">No policy rules configured</p>
        <% else %>
          <div class="space-y-2 mb-3">
            <div :for={rule <- @policy_rules} class="flex items-center justify-between bg-raised/50 rounded px-3 py-2">
              <div>
        ...
        <form phx-submit="add_policy_rule" class="flex items-center gap-2">
          <input type="text" name="name" placeholder="Rule name" class="flex-1 bg-raised border border-border-subtle rounded px-2 py-1 text-xs text-high placeholder-muted focus:border-interactive focus:ring-0" />
          <input type="text" name="condition" placeholder="Condition" class="flex-1 bg-raised border border-border-subtle rounded px-2 py-1 text-xs text-high placeholder-muted focus:border-interactive focus:ring-0" />
          <input type="text" name="action" placeholder="Action" class="flex-1 bg-raised border border-border-subtle rounded px-2 py-1 text-xs text-high placeholder-muted focus:border-interactive focus:ring-0" />
          <button type="submit" class="px-3 py-1 text-xs bg-highlight hover:bg-highlight text-high rounded transition">Add</button>
        </form>
      </div>
