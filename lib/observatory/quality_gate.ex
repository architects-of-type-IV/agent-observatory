defmodule Observatory.QualityGate do
  @moduledoc """
  ...
  defp run_gate_command(command, cwd) do
    timeout = Application.get_env(:observatory, __MODULE__, []) |> Keyword.get(:timeout, @default_timeout)
  ...
  defp nudge_agent(session_id, task_id, command, output) do
    ...
    message =
      "[Observatory Quality Gate FAILED]#{task_ref}\n" <>
        "Command: #{command}\n" <>
        "Output:\n#{truncated}\n\n" <>
        "Fix the issues and re-run the gate before reporting completion."

    # Try tmux first, fall back to mailbox
    case Tmux.deliver(session_id, %{content: message, from: "observatory", type: :quality_gate}) do
      :ok ->
        :ok
