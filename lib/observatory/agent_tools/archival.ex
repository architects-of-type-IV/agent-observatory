defmodule Observatory.AgentTools.Archival do
  @moduledoc """
  ...
  actions do
    action :archival_memory_insert, :map do
      description "Store information in your long-term archival memory. Tag entries for easier retrieval."
        ...
        case MemoryStore.archival_memory_search(args.agent_name, args.query, tags: tags, page: page) do
          {:ok, results} ->
            {:ok, Enum.map(results, fn e ->
              %{"id" => e.id, "content" => e.content, "tags" => e.tags || [], "timestamp" => e.timestamp}
            end)}
