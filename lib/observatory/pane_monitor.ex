defmodule Observatory.PaneMonitor do
  @moduledoc """
  ...
  Subscribes to the heartbeat and periodically captures pane output to detect:
    - `OBSERVATORY_DONE: <summary>` -- agent signals task completion
    - `OBSERVATORY_BLOCKED: <summary>` -- agent signals it needs help
  ...
  defp check_done_signal(agent, text, state) do
    case Regex.run(~r/OBSERVATORY_DONE:\s*(.+)/, text) do
      [_, summary] ->
  ...
  defp check_blocked_signal(agent, text, state) do
    case Regex.run(~r/OBSERVATORY_BLOCKED:\s*(.+)/, text) do
      [_, reason] ->
