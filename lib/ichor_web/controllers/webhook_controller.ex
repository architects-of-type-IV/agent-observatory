defmodule IchorWeb.WebhookController do
  ...
  def create(conn, %{"webhook_id" => webhook_id}) do
    with {:ok, signature} <- get_signature(conn),
      ...
      {:error, :missing_signature} ->
        ...
        |> json(%{"status" => "error", "reason" => "missing X-Ichor-Signature header"})
  ...
  defp get_signature(conn) do
    case get_req_header(conn, "x-ichor-signature") do
      [sig | _] -> {:ok, sig}
      [] -> {:error, :missing_signature}
    end
  end
  ...
  defp get_secret(webhook_id) do
    ...
    secrets = Application.get_env(:ichor, :webhook_secrets, %{})

    case Map.get(secrets, webhook_id) do
      nil -> {:ok, Application.get_env(:ichor, :webhook_default_secret, "default-secret")}
      secret -> {:ok, secret}
    end
  end
end
