defmodule IchorWeb.Components.AgentsComponents do
  @moduledoc """
  Agents/teams view component for the Ichor dashboard.
  """
  ...
  def agents_view(assigns) do
    ...
    <div class="p-4">
      ...
      <div :if={@teams != []} class="grid grid-cols-2 gap-4 min-w-0">
        <div
          :for={team <- @teams}
          class="p-4 rounded-lg border border-border bg-base/50"
        >
          <div class="mb-3">
            <div class="flex items-center justify-between mb-1">
              <h3 class="text-sm font-semibold text-high">{team.name}</h3>
            </div>
            <p :if={team.description} class="text-xs text-low mb-2">{team.description}</p>
          ...
          <div class="space-y-2">
            <h4 class="text-xs font-semibold text-low uppercase tracking-wider">Members</h4>
            <div
              :for={member <- team.members}
              phx-click="select_agent"
              phx-value-id={member[:agent_id]}
              class={"p-2 rounded bg-raised/50 border hover:border-border-subtle transition cursor-pointer #{if member[:status] == :idle, do: "opacity-60", else: ""} #{if @selected_agent && @selected_agent[:agent_id] == member[:agent_id], do: "border-interactive/40 bg-raised/80", else: "border-border"}"}
            >
              <div class="flex items-center gap-2 mb-1">
                <span class={"w-2 h-2 rounded-full shrink-0 #{member_status_color(member)}"}></span>
                <span class="text-xs font-semibold text-high">{member[:name] || "?"}</span>
                <span :if={member[:agent_type]} class="text-xs text-muted ml-auto">
                  {member[:agent_type]}
                </span>
                ...
                <button
                  ...
                  class="inline-flex items-center justify-center px-1.5 min-w-[1.25rem] h-4 text-xs font-semibold text-white bg-info rounded-full hover:bg-info transition"
                  title="View tasks"
                >
                  {task_count}
                </button>
              ...
              <div class="flex items-center gap-2 ml-4 text-xs text-low mb-1">
                <span :if={member[:cwd]}>
              ...
              <div :if={member[:current_tool]} class="ml-4 mb-1 text-xs text-info">
              ...
              <div :if={member[:failure_rate] && member[:failure_rate] > 0} class="ml-4 mb-1">
                ...
                <span class={"text-xs px-1.5 py-0.5 rounded #{if fail_pct > 10, do: "bg-error/20 text-error", else: "bg-brand/20 text-brand"}"}>
                  {fail_pct}% failures
                </span>
              </div>
              ...
              <div :if={member[:agent_id]} class="ml-4 space-y-1">
                ...
                >
                  <form phx-submit="send_agent_message" class="flex gap-1">
                    ...
                    <input
                      ...
                      class="flex-1 bg-base border border-border-subtle rounded px-2 py-1 text-xs text-high placeholder-muted focus:border-interactive focus:ring-0 focus:outline-none"
                    />
                    <button
                      type="submit"
                ...
                <div class="flex gap-1">
                  <button
                    ...
                    phx-value-session_id={member[:agent_id]}
                    class="flex-1 px-2 py-1 bg-highlight/50 text-default rounded text-xs hover:bg-highlight transition"
                    title="View events"
                  >
                    Events
                  </button>
