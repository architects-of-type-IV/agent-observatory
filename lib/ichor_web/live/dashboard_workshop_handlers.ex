defmodule IchorWeb.DashboardWorkshopHandlers do
  ...
  def handle_event("ws_add_agent_from_type", %{"id" => id}, socket) do
    case Ichor.Workshop.AgentType.by_id(id) do
      {:ok, type} ->
        agent = new_agent(socket, %{
  ...
  def handle_event("ws_launch_team", _, socket) do
    ...
    case Ichor.Fleet.Team.create_team(team_name, strategy: String.to_existing_atom(socket.assigns.ws_strategy)) do
      {:ok, _} ->
        ...
        launched =
          Enum.count(order, fn a ->
            match?({:ok, _}, Ichor.Fleet.Agent.launch(%{
