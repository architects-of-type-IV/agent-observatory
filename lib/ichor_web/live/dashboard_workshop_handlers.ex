defmodule IchorWeb.DashboardWorkshopHandlers do
  ...
  def handle_event("ws_add_spawn_link", %{"from" => from, "to" => to}, socket) do
    ...
    if already?,
      do: {:noreply, socket},
        ...
        {:noreply,
         socket |> assign(:ws_spawn_links, links ++ [%{from: from, to: to}]) |> save_and_push()}
  end
  ...
  def handle_event("ws_add_comm_rule", %{"from" => from, "to" => to, "policy" => policy}, socket) do
    ...
    if exists?,
      do: {:noreply, socket},
        ...
        {:noreply,
         socket
         |> assign(:ws_comm_rules, rules ++ [%{from: from, to: to, policy: policy, via: nil}])
         |> save_and_push()}
  end
  ...
  def handle_event("ws_remove_spawn_link", %{"index" => idx}, socket) do
    {:noreply,
     socket
     |> assign(:ws_spawn_links, List.delete_at(socket.assigns.ws_spawn_links, to_int(idx)))
     |> save_and_push()}
  end
  ...
  def handle_event("ws_remove_comm_rule", %{"index" => idx}, socket) do
    {:noreply,
     socket
     |> assign(:ws_comm_rules, List.delete_at(socket.assigns.ws_comm_rules, to_int(idx)))
     |> save_and_push()}
  end
  ...
  def handle_event("ws_launch_team", _, socket) do
    ...
    case Team.create_team(team_name,
           strategy: String.to_existing_atom(socket.assigns.ws_strategy)
         ) do
      {:ok, _} ->
        ...
        {:noreply,
         flash(
           socket,
           :info,
           "Team #{team_name} launched with #{launched}/#{length(agents)} agents"
         )}
  ...
  defp merge_agent(agent, params) do
    %{
      ...
      | name: params["name"] || agent.name,
        capability: params["capability"] || agent.capability,
        model: params["model"] || agent.model,
        permission: params["permission"] || agent.permission,
        persona: params["persona"] || agent.persona,
