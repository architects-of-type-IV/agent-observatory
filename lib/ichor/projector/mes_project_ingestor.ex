defmodule Ichor.Projector.MesProjectIngestor do
  @moduledoc """
    ...
    case extract_mes_payload(data) do
      ...
      :skip ->
        ...
        case AgentId.parse(from) do
          {:ok, %AgentId{kind: :mes}} ->
            Logger.debug(
              "[MesProjectIngestor] Skipped MES message from #{from}: missing required fields"
            )
  ...
  defp ingest_project(payload, data) do
    ...
    case create_if_unique(attrs) do
      ...
      {:error, reason} ->
        Logger.warning("[MES.MesProjectIngestor] Failed to ingest: #{inspect(reason)}")
    end
  end
  ...
  defp check_and_create(existing, attrs) do
    if Enum.any?(existing, &(&1.plugin == attrs.plugin && &1.status != :failed)) do
      Logger.warning("[MES.MesProjectIngestor] Duplicate plugin rejected: #{attrs.plugin}")
      {:error, :duplicate_plugin}
    else
