defmodule Ichor.Events.EventStream do
    ...
    case ingest(raw_map) do
      {:ok, event} ->
        unless tombstoned?(event.session_id) do
          Events.emit(Event.new("events.hook.ingested", event.session_id, %{event: event}))
  ...
  def publish_fact(name, attrs \\ %{}) when is_atom(name) and is_map(attrs) do
    Events.emit(Event.new("events.hook.ingested", nil, %{name: name, attrs: attrs}))
  ...
  def handle_info(:check_heartbeats, state) do
    ...
    Enum.each(evicted_ids, fn agent_id ->
      Events.emit(Event.new("fleet.agent.evicted", agent_id, %{session_id: agent_id}))
  ...
  defp ingest_event(event) do
    ...
    Events.emit(Event.new("agent.event", agent_id, %{event: event, scope_id: agent_id}))
