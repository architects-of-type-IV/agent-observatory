    ...
    session_nodes =
      Enum.map(all_sessions, fn s ->
        ...
        %{
          ...
          state: status,
          label: team_info[:role] || s.source_app || Ichor.Gateway.AgentRegistry.AgentEntry.short_id(s.session_id),
      ...
      |> Enum.flat_map(fn team ->
        ...
        |> Enum.map(fn m ->
          %{
            ...
            label: m[:name] || m[:agent_type] || Ichor.Gateway.AgentRegistry.AgentEntry.short_id(m[:agent_id] || ""),
