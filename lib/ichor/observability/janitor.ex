defmodule Ichor.Observability.Janitor do
  @moduledoc """
  Periodically purges old events from SQLite to keep the database small.
  Runs every 6 hours and deletes events older than @retention_days.
  """
  use GenServer
  require Logger
  import Ecto.Query

  @retention_days 7
  @interval_ms :timer.hours(6)

  @doc false
  @spec start_link(keyword()) :: GenServer.on_start()
  def start_link(opts), do: GenServer.start_link(__MODULE__, opts, name: __MODULE__)

  @impl true
  def init(_opts) do
    # First purge after 30 seconds so startup is not blocked.
    Process.send_after(self(), :purge, 30_000)
    {:ok, %{}}
  end

  @impl true
  def handle_info(:purge, state) do
    Process.send_after(self(), :purge, @interval_ms)

    try do
      cutoff = DateTime.add(DateTime.utc_now(), -@retention_days, :day)

      {events, _} =
        from(e in "events", where: e.inserted_at < ^cutoff)
        |> Ichor.Repo.delete_all()

      {sessions, _} =
        from(s in "sessions", where: s.inserted_at < ^cutoff)
        |> Ichor.Repo.delete_all()

      if events > 0 or sessions > 0 do
        Logger.info(
          "Observability.Janitor: purged #{events} events, #{sessions} sessions older than #{@retention_days} days"
        )
      end
    rescue
      e -> Logger.warning("Observability.Janitor: purge error: #{inspect(e)}")
    catch
      :exit, _ -> :ok
    end

    {:noreply, state}
  end
end
