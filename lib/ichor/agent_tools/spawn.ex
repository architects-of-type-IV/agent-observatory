  ...
  actions do
    action :spawn_agent, :map do
      ...
      argument :capability, :string do
        allow_nil? true
        description "Agent role: builder (read+write), scout (read-only), lead (full), reviewer (read+write). Defaults to builder."
      end
      ...
      argument :team_name, :string do
        allow_nil? true
        description "Team to join. Creates the team if it doesn't exist. Omit for standalone agent."
      end
        ...
        case Ichor.AgentSpawner.spawn_agent(opts) do
          {:ok, result} ->
            {:ok, %{
              ...
              "note" => "Agent is running in tmux. Use send_message to communicate. Check fleet panel for status."
            }}
    ...
    action :stop_agent, :map do
      description "Stop a previously spawned agent. Terminates both the tmux session and BEAM process."
