defmodule Ichor.AgentTools.Memory do
  @moduledoc """
  ...
  actions do
    action :read_memory, :map do
      description "Read your core memory blocks. Returns all blocks pinned to your context. Call this first when starting a session."
            ...
            blocks =
              Enum.map(memory.blocks, fn b ->
                %{"label" => b.label, "description" => b.description,
                  "value" => b.value, "read_only" => b.read_only}
              end)

            {:ok, %{"agent" => name, "blocks" => blocks,
              "recall_count" => memory.recall_count, "archival_count" => memory.archival_count}}
    ...
    action :memory_replace, :map do
      ...
      run fn input, _context ->
        ...
        case MemoryStore.memory_replace(args.agent_name, args.block_label, args.old_text, args.new_text) do
          {:ok, block} -> {:ok, %{"status" => "replaced", "block" => args.block_label, "new_value" => block.value}}
          {:error, :text_not_found} -> {:error, "Text not found in block '#{args.block_label}'."}
          {:error, :read_only} -> {:error, "Block '#{args.block_label}' is read-only."}
          {:error, :exceeds_limit} -> {:error, "Replacement would exceed the block's character limit."}
    ...
    action :memory_insert, :map do
      ...
      run fn input, _context ->
        ...
        case MemoryStore.memory_insert(args.agent_name, args.block_label, args.position, args.text) do
          {:ok, block} -> {:ok, %{"status" => "inserted", "block" => args.block_label, "new_value" => block.value}}
          {:error, :read_only} -> {:error, "Block '#{args.block_label}' is read-only."}
          {:error, :exceeds_limit} -> {:error, "Insert would exceed the block's character limit."}
          {:error, reason} -> {:error, "Failed: #{inspect(reason)}"}
        end
      end
    end
    ...
    action :memory_rethink, :map do
      ...
      run fn input, _context ->
        ...
        case MemoryStore.memory_rethink(args.agent_name, args.block_label, args.new_value) do
          {:ok, block} -> {:ok, %{"status" => "rewritten", "block" => args.block_label, "new_value" => block.value}}
          {:error, :read_only} -> {:error, "Block '#{args.block_label}' is read-only."}
          {:error, :exceeds_limit} -> {:error, "New content exceeds the block's character limit."}
          {:error, reason} -> {:error, "Failed: #{inspect(reason)}"}
        end
      end
    end
