defmodule Ichor.Projects.Runtime do
  ...
  @impl true
  def init(_opts) do
    project_state = Catalog.initial_state()
  ...
  def handle_call({:set_active_project, key}, _from, state) do
    case Catalog.set_active_project(state, key) do
      {:ok, next_state} ->
  ...
  def handle_call({:add_project, key, path}, _from, state) do
    case Catalog.add_project(state, key, path) do
      {:ok, next_state} ->
        next_state = refresh_tasks(next_state)
        broadcast(next_state)
        {:reply, :ok, next_state}
  ...
  def handle_info(%Message{name: :new_event, data: %{event: event}}, state) do
    {next_state, changed?} = Catalog.register_cwd(state, event.cwd)
  ...
  def handle_info(:poll_tasks, state) do
    ...
    {next_state, projects_changed?} = Catalog.refresh_discovered_projects(state)
  ...
  defp do_health_check(state) do
    HealthReport.run(state, Catalog.active_project_path(state))
  end
