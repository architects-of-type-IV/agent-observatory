...
This section creates `assets/js/hooks/topology_map.js`, the `Observatory.Gateway.TopologyBuilder` Elixir module, and establishes the `"gateway:topology"` PubSub broadcast contract. Together these six tasks cover FR-8.9 through FR-8.13, add the basic zoom/pan controls specified in ADR-016's Phase 1 scope, and complete the full data path from ETS DAG to Canvas pixel. No React or D3 dependency is introduced; the Phase 2 migration trigger (>3 interactive requirements needing >200 lines of custom Canvas JS) is explicitly not met at this phase boundary.
...
- [ ] 3.3.2.2 Implement `render()` in `TopologyMap`: first, run one iteration of a simple force-directed layout — for each pair of nodes apply a repulsive force `F_repel = 500 / distance^2` pushing nodes apart (capped at 10px displacement per frame to prevent explosion), and for each edge apply an attractive spring force `F_attract = (distance - 80) * 0.01` pulling connected nodes toward an ideal edge length of 80px; update each node's `x` and `y` by the net force vector (nodes without positions are initialized to random coordinates within the canvas bounds on first render); then clear the canvas with `this.ctx.clearRect(0, 0, this.canvas.width, this.canvas.height)`; iterate `this.nodes` and for each node: resolve the fill color as `NODE_COLORS[node.state] || NODE_COLORS.idle`; set `this.ctx.fillStyle = color`; call `this.ctx.beginPath()` then `this.ctx.arc(node.x, node.y, 12, 0, Math.PI * 2)` then `this.ctx.fill()`; for `alert_entropy` nodes, apply a flashing effect by modulating `this.ctx.globalAlpha` using `0.5 + 0.5 * Math.sin(Date.now() / 300)` before drawing, then restoring `globalAlpha` to `1.0` afterward; after drawing nodes, iterate `this.edges` and draw each as a line from the source node's `(x, y)` to the target node's `(x, y)` using `this.ctx.moveTo`/`this.ctx.lineTo`/`this.ctx.stroke()` `done_when: "mix compile --warnings-as-errors"`
...
- [ ] 3.3.4.1 Create `lib/observatory/gateway/topology_builder.ex` with `use GenServer`; in `init/1`, initialize `state` with `%{sessions: MapSet.new()}`; implement a public `subscribe_to_session(session_id)` function that calls `GenServer.cast(__MODULE__, {:subscribe, session_id})`; `handle_cast({:subscribe, session_id}, state)` subscribes to `Phoenix.PubSub` topic `"session:dag:#{session_id}"` if not already in `state.sessions`, adds `session_id` to the MapSet, and returns `{:noreply, updated_state}`; implement `handle_info(%{event: "dag_delta", session_id: session_id} = _delta, state)` that calls `CausalDAG.get_session_dag(session_id)` and pattern-matches on the result; `CausalDAG.insert/2` must call `TopologyBuilder.subscribe_to_session(session_id)` on the first node insertion for a new session (inside `ensure_session_table/1` when a new table is created) `done_when: "mix compile --warnings-as-errors"`
...
- [ ] 3.3.4.3 In `lib/observatory/mesh/causal_dag.ex`, inside the `ensure_session_table(session_id)` function (created in task 3.1.1), add a call to `Observatory.Gateway.TopologyBuilder.subscribe_to_session(session_id)` ONLY when a new table is created (not on subsequent calls for existing sessions); this is a one-line addition that does NOT modify `broadcast_delta/4` or any other existing function — it only extends the new-table creation path; guard the call with `Code.ensure_loaded?(Observatory.Gateway.TopologyBuilder)` so CausalDAG compiles independently of TopologyBuilder `done_when: "mix compile --warnings-as-errors"`
...
### 3.3.6 Basic Zoom & Pan Controls

- [ ] **Task 3.3.6 Complete**
- **Governed by:** ADR-016
- **Parent UCs:** UC-0238

ADR-016 scopes Phase 1 to "force-directed layout, node state colors, edge click, basic zoom/pan." This task adds the minimum zoom and pan controls using Canvas 2D transforms. No external library is used.

- [ ] 3.3.6.1 In `topology_map.js`, add state fields in `mounted()`: `this.scale = 1.0`, `this.offsetX = 0`, `this.offsetY = 0`, `this.isPanning = false`, `this.lastPanX = 0`, `this.lastPanY = 0`; add a `wheel` event listener on `this.canvas` for zoom: `this.canvas.addEventListener('wheel', this.handleWheel.bind(this), { passive: false })`; implement `handleWheel(e)` that calls `e.preventDefault()`, computes zoom delta as `const zoomFactor = e.deltaY > 0 ? 0.9 : 1.1`, clamps `this.scale` between `0.1` and `5.0`, and adjusts `this.offsetX`/`this.offsetY` to zoom toward the mouse position `done_when: "mix compile --warnings-as-errors"`
- [ ] 3.3.6.2 Add `mousedown`, `mousemove`, and `mouseup` event listeners on `this.canvas` for pan: `mousedown` sets `this.isPanning = true` and records `this.lastPanX = e.clientX`, `this.lastPanY = e.clientY`; `mousemove` (when `this.isPanning`) updates `this.offsetX += e.clientX - this.lastPanX` and `this.offsetY += e.clientY - this.lastPanY` and records the new last position; `mouseup` sets `this.isPanning = false`; in `render()`, apply the transform before drawing: `this.ctx.save(); this.ctx.translate(this.offsetX, this.offsetY); this.ctx.scale(this.scale, this.scale);` and call `this.ctx.restore()` after all drawing is complete `done_when: "mix compile --warnings-as-errors"`
- [ ] 3.3.6.3 Update `handleClick(e)` to account for the current zoom/pan transform when computing canvas-local coordinates: transform `(x, y)` by the inverse of the current `scale` and `offset` — `const canvasX = (x - this.offsetX) / this.scale; const canvasY = (y - this.offsetY) / this.scale;` — and use `canvasX`/`canvasY` for node and edge hit detection instead of the raw `x`/`y`; this ensures clicks remain accurate after zoom/pan `done_when: "mix compile --warnings-as-errors"`
...
**Cross-phase dependency:** Tasks 3.6.1 and 3.6.2 modify `lib/observatory/gateway/schema_interceptor.ex` and `test/observatory/gateway/schema_interceptor_test.exs`, which are Phase 2 artifacts. These tasks MUST NOT begin until Phase 2 is fully complete (all Phase 2 tasks at status `completed` in tasks.jsonl). The Phase-to-DAG script detects intra-phase file overlaps but not cross-phase overlaps — this constraint must be enforced manually by adding `blocked_by` to the Phase 2 final task ID when generating tasks.jsonl.
